...
class KWinDriver {
    ...
    bindShortcut() {
        ...
        this.shortcuts
            .getCycleMaster()
            .activated.connect(callbackShortcut(Shortcut.CycleMaster));
        this.shortcuts
            .getCycleMasterBack()
            .activated.connect(callbackShortcut(Shortcut.CycleMasterBack));
    ...
    onShortcut(ctx, input, data) {
        ...
        switch (input) {
            ...
            case Shortcut.SetMaster:
                ...
                break;
            case Shortcut.CycleMaster:
                this.engine.cycleMaster(ctx, +1);
                break;
            case Shortcut.CycleMasterBack:
                this.engine.cycleMaster(ctx, -1);
                break;
...
class TilingEngine {
    ...
    cycleMaster(ctx, step) {
        const tiles = this.windows.getVisibleTileables(ctx.currentSurface);
        if (tiles.length < 2)
            return;
        const master = tiles[0];
        const last = tiles[tiles.length - 1];
        if (step > 0) {
            // Promote next window to master; old master rotates to the back.
            // [A,B,C,D] -> [B,C,D,A]
            this.windows.move(master, last, true);
        }
        else {
            // Promote last window to master; everything else shifts back.
            // [A,B,C,D] -> [D,A,B,C]
            this.windows.move(last, master, false);
        }
        const newTiles = this.windows.getVisibleTileables(ctx.currentSurface);
        ctx.currentWindow = newTiles[0];
    }
